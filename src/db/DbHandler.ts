...
export class DbHandler {
    ...
    async createGlobalRatesForAllEndpoints() {
        for (const [endpointName, endpointConfig] of Object.entries(config.ENDPOINTS)) {
            await this.client.set(`${endpointName}:GlobalRates`, endpointConfig.RATE_LIMIT.GLOBAL.MAX_STORED);
            getLogger().simpleLog("success", `Global Rates Created for ${endpointName}`);
        }
    }
    ...
    async createUser(discordId: string, endpointName: string, maxStored: number) {
        ...
        if (!username) {
            getLogger().simpleLog("warn", `Could not find discord username for ${discordId}`);
            return false;
        }
    ...
    async getGlobalRates(endpointName: string) {
        ...
        if (!rates) {
            getLogger().simpleLog("warn", `Error Fetching Global Rates for ${endpointName}`);
            return null;
        }
    ...
    async updateGlobalRates(endpointName: string, type: "give" | "take") {
        ...
        switch (type) {
            case "give":
                ...
                newRates = tmpRates >= globalConfig.MAX_STORED ? globalConfig.MAX_STORED : tmpRates;
                break;
            case "take":
                tmpRates = rates - 1;
    ...
    async updateUserRates(discordId: string, endpointName: string, type: "give" | "take") {
        ...
        switch (type) {
            case "give":
                ...
                newRates = tmpRates >= userConfig.MAX_STORED ? userConfig.MAX_STORED : tmpRates;
                break;
            case "take":
                tmpRates = rates - 1;
                newRates = tmpRates <= 0 ? 0 : tmpRates;
        ...
        await this.client.hSet(`${endpointName}:user:${discordId}`, "rates", newRates);
    }

    async getUserData(discordId: string, endpointName: string) {
        const key = `${endpointName}:user:${discordId}`;
        const exists = Boolean(await this.client.exists(key));
        if (!exists) {
            const maxStored = config.ENDPOINTS[endpointName].RATE_LIMIT.USER.MAX_STORED;
            const created = await this.createUser(discordId, endpointName, maxStored);
            if (!created) {
                throw new Error(`Failed to create user for Discord ID: ${discordId} on ${endpointName}`);
            }
        }
    ...
    async getHogMembers() {
        return this.client.sMembers("members");
    }
...
export function getDbHandler(): DbHandler {
    if (!dbHandler) throw new Error("Db Handler not initialized. Call initDbHandler() first.");
    return dbHandler;
}
