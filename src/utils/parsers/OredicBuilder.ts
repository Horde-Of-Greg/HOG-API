...
import type { AstNode, BuildOptions, OredicMatches, OrNode, PatternNode } from "../../types/parsing";
import { NodeNames, OperatorChar } from "../../types/parsing";
...
export class OredicBuilder extends ErrorProne {
    ...
    buildFromMatches(matches: OredicMatches, options: BuildOptions): AstNode[] | StandardError {
        ...
        for (const buildStrategy of pipeline) {
            ...
            if (this.isError(result)) {
                return this.propagateError(result, "Build strategy failed", "buildFromMatches");
            }
            if (result) {
        ...
        if (candidates.size === 0) {
            return this.setError(500, "No valid AST candidates generated", "buildFromMatches");
        }
    ...
    private shortenedDisjuntion(matches: OredicMatches): OrNode | StandardError {
        ...
        for (const match in matches) {
            ...
            if (this.isError(nodeCandidate)) {
                return this.propagateError(nodeCandidate, "Failed to parse pattern", "shortenedDisjuntion");
            }
            if (nodeCandidate.type === NodeNames.OPERATOR) {
                return this.setError(500, "Parsed a pattern as an operator", "shortenedDisjuntion");
            }
