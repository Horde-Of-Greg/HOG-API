...
import { NodeNames } from "../../types/parsing";
...
export class OredicMatcher extends ErrorProne {
    ...
    isUniqueMatch(pattern: string, targetIndex: number): boolean {
        ...
        for (let radius = 1; radius <= maxRadius; radius *= 2) {
            for (let offset = -radius; offset <= radius; offset++) {
                ...
                if (checkIndex === targetIndex || checkIndex < 0 || checkIndex >= this.dump.length) {
                    continue;
                }
    ...
    private applyConjunction(node: MatcherAndNode): OredicNode {
        ...
        if (nonNegatedCount === 0) {
            this.setError(500, "AND operation requires at least one non-negated child", "applyConjunction");
            return this.newOredicNode(null);
        }
