...
import type { GrokTokenizeReq } from "../../types/ai";
...
async function makeTokenizeReq(text: string, model: string): Promise<GrokTokenizeReq | null> {
    const response = await fetch(getGrokClient().client.baseURL + "/tokenize-text", {
        method: "POST",
        headers: {
