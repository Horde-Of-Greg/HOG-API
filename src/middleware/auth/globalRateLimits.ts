...
export const globalRateLimits = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const isAllowed = await checkRateLimits(req.body.userId, aiModel, modelConfig.RATE_LIMIT.USER.WHITELIST);
    if (!isAllowed) {
        getLogger().simpleLog("warn", `Global Rate Limit Reached for ${aiModel}`);
        res.status(429).json({ error: "You're Being Rate Limited by the Global Rate Limit" });
        return;
    }
...
async function checkRateLimits(dcUserId: string, modelName: string, whitelist: string[]) {
