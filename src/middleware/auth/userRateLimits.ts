...
export const userRateLimits = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const isAllowed = await checkRateLimits(userId, aiModel, modelConfig.RATE_LIMIT.USER.WHITELIST);

    if (!isAllowed) {
        const username = await getDbHandler().getUsername(userId, aiModel);
        getLogger().simpleLog("warn", `User: ${username} got rate limited on ${aiModel}`);
