...
export const filterBody = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
...
function filterAndWarn(text: string, username: string) {
    FILTERS.forEach((filter) => {
        if (filter.pattern.test(text)) {
            getLogger().simpleLog("warn", `${filter.description} (${filter.severity}) - User: ${username}`);
        }
    });
}
