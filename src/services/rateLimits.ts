...
export async function startUserRateLimitIncrements() {
    for (const [endpointName, endpointConfig] of Object.entries(config.ENDPOINTS)) {
        ...
        const interval = setInterval(async () => {
            try {
                const keys = await getDbHandler().getAllUserKeysForEndpoint(endpointName);
                ...
                getLogger().simpleLog("error", `Failed to increment user rates for ${endpointName}: ${err}`);
            }
        }, intervalMs);
...
export function stopGlobalRateLimitIncrement() {
    for (const [key, interval] of rateLimitIntervals.entries()) {
        if (key.endsWith(":global")) {
            ...
            getLogger().simpleLog("info", `Global rate limit increment stopped for ${endpointName}`);
        }
    }
}
...
export function stopUserRateLimitIncrements() {
    for (const [key, interval] of rateLimitIntervals.entries()) {
        if (key.endsWith(":user")) {
            ...
            getLogger().simpleLog("info", `User rate limit increments stopped for ${endpointName}`);
        }
    }
}
