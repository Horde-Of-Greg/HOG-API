...
export class Logger {
    ...
    progressBar(
        ...
        const progress = this.buildProgressBar(currentCount, totalCount, timerId, barWidth, label);
    ...
    private formatLogPrefix(type: LogType | "format", timestamp: string): string {
        ...
        const padding = " ".repeat(FormattingConstant.MAX_TYPE_LENGTH - type.length);
    ...
    private createBar(currentCount: number, totalCount: number, barWidth: number): string {
        const filledWidth = Math.floor((currentCount / totalCount) * barWidth);
        const emptyWidth = Math.max(0, barWidth - filledWidth);

        const filled = (FormattingConstant.PROGRESS_BAR_FILLED as string).repeat(filledWidth);
        const empty = (FormattingConstant.PROGRESS_BAR_EMPTY as string).repeat(emptyWidth);

        return `${filled}${empty}`;
    }

    private createTimeInfo(currentCount: number, totalCount: number, timerId: string): string {
        const elapsedMilliseconds = queryTimer(timerId).getTime("ms").raw;
        const elapsedSeconds = Math.floor(elapsedMilliseconds / FormattingConstant.MILLISECONDS_PER_SECOND);
        ...
        if (isInProgress) {
            const remainingMilliseconds = (elapsedMilliseconds / currentCount) * (totalCount - currentCount);
    ...
    private formatTime(totalSeconds: number): string {
        const minutes = Math.floor(totalSeconds / FormattingConstant.SECONDS_PER_MINUTE);
